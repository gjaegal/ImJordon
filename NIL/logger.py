...
class Logger:
    ...
    def log_trajs_as_videos(
        ...
        seg_videos = [np.transpose(p["image_obs"], [0, 3, 1, 2]) for p in trajs]
        ...
        seg_videos = np.stack(seg_videos[:max_videos_to_save], 0)
        self.log_video(seg_videos, video_title + "_seg", step, fps=fps)
